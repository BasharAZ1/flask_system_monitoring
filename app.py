...
def collect_system_info():
 with app.app_context():
    ...
    d = Disk(used=disk_usage.used / (1024**3),free=disk_usage.free / (1024**3),usage_percent=disk_usage.percent)
    ...
    for proc in psutil.process_iter(attrs=['pid', 'name', 'status', 'create_time']):
        try:
            ...
            process = Active_processes(pid=pid, name=name, status=status, start_date=start_date)
